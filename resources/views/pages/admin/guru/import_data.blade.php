...
@section('table-name', 'Import Data Guru')
...
@section('content')
    ...
    {{-- End Modal Preview --}}
    {{-- Modal Import --}}
    <div class="absolute z-50 hidden modal" id="modal-id_form_add" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="absolute inset-0 transition-opacity bg-black bg-opacity-50 modal-overlay">
            </div>
                        ...
                        <div class="p-5">
                            ...
                            <form action="{{ route('admin.guru.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <input
                                    ...
                                    id="file" type="file" name="file" required>
                                <button type="submit"
                                    class="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md">
                                    Import Data Guru
                                </button>
                            </form>
                        </div>
    ...
    <script>
        ...
        function populateTable(data) {
            ...
            for (var i = 1; i < data.length; i++) {
                ...
                var nama = data[i][0]; // Kolom A
                var mapel1 = data[i][1]; // Kolom B
                var mapel2 = data[i][2]; // Kolom C
                var jabatan = data[i][3]; // Kolom D
                var niy = data[i][4]; // Kolom E
                var password = data[i][5]; // Kolom F

                var cells = [nama, mapel1, mapel2, jabatan, niy, password];
